...
def make_fastqs(args):
    """
    splits the bamfile into chunks and calls /renamer.py on these chunks in parallel
    """
...
def remap(args, region_fastqs, all_fastqs):
    """
    calls minimap on the chunked fastqs.
    it merges all chunks generated by the same threat though...
    """
    print("remapping")
    # run minimap2
    minimap_tmp_files = []
...
def retag(args, minimap_tmp_files):
    """
    calling retag on every sam file generated by minimap.
    also sorting, merging and indexing
    """
